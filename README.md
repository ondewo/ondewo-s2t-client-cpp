...
`ondewo-s2t-client-cpp` is the C++ gRPC client library for the
[ONDEWO S2T API](https://github.com/ondewo/ondewo-s2t-api) - ONDEWO's Speech-to-Text service. It is a
...
| `ondewo-s2t-api/`                   | submodule - the `.proto` source of truth                                 |
...
C++ has no package registry, so every [GitHub release](https://github.com/ondewo/ondewo-s2t-client-cpp/releases)
carries the built package as an asset: `ondewo_s2t_client-<version>-<platform>.tar.gz`, plus a
...
gh release download "${version}" --repo ondewo/ondewo-s2t-client-cpp --pattern "${archive}*"
...
```cmake
find_package(ondewo_s2t_client CONFIG REQUIRED)
target_link_libraries(my_app PRIVATE ondewo::ondewo_s2t_client)
```

```shell
cmake -S . -B build -DCMAKE_PREFIX_PATH=/opt/ondewo_s2t_client-${version}-${platform}
```
...
set(ONDEWO_LIBRARY_NAME ondewo_s2t_client CACHE STRING "" FORCE)
...
target_link_libraries(my_app PRIVATE ondewo_s2t_client)
```
...
```shell
git clone https://github.com/ondewo/ondewo-s2t-client-cpp.git
cd ondewo-s2t-client-cpp
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release \
  -DONDEWO_LIBRARY_NAME=ondewo_s2t_client \
...
```shell
git clone https://github.com/ondewo/ondewo-s2t-client-cpp.git   ## Clone repository
cd ondewo-s2t-client-cpp                                        ## Change into repo directory
...
generated header you need (`api/ondewo/s2t/<file>.grpc.pb.h`) to keep compile times down.
...
int main() {
  ...
  auto channel = grpc::CreateChannel("s2t.ondewo.com:443", grpc::SslCredentials({}));

  // Replace Services/Request/Response with the service you need - see the API documentation.
  auto stub = ondewo::s2t::Services::NewStub(channel);

  grpc::ClientContext context;
  context.AddMetadata("authorization", "Bearer " + std::string(getenv("ONDEWO_TOKEN")));

  ondewo::s2t::Request request;
  ondewo::s2t::Response response;
...
docker run \
  -v $(pwd):/input-volume \
  -v $(pwd):/output-volume \
  ondewo-cpp-proto-compiler ondewo-s2t-api ondewo ondewo_s2t_client
```
...
- In the output volume it deletes only what it owns - `api/`, `include/ondewo_s2t_client/`,
  `lib/libondewo_s2t_client.a` and `lib/cmake/ondewo_s2t_client/` - so a proto that was renamed or
...
- `check_build` asserts that every `.proto` under `ondewo-s2t-api/ondewo` produced a matching `.pb.h`.
- `smoke_test` writes a throwaway project that does `find_package(ondewo_s2t_client CONFIG REQUIRED)`,
  includes `public-api.h` and links `ondewo::ondewo_s2t_client`, then compiles and runs it. That is the
