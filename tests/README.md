...
| `test_typed_api.cc` | **yes** | Assertions against concrete `ondewo::s2t` C++ types |
...
- `optional string language` — a proto3 *explicit presence* field — still reports `has_...()`
  after an empty-string round trip, while the plain `string s2t_pipeline_id` correctly keeps its
  zero value off the wire. This is the bug class that broke the Angular client.
